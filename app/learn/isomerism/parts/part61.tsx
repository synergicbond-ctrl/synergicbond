...
function PyridineTautomer({ kind }: { kind: Tautomer }) {
  ...
  return <figure className="rounded-lg border border-white/10 bg-slate-950/50 p-3">
...
function Pair({ left, right, conclusion, note }: { left: Tautomer; right: Tautomer; conclusion: string; note: string }) {
  return <article className="rounded-lg border border-cyan-300/20 bg-cyan-300/[.045] p-4">
...
export default function IsomerismPart61() {
  return <main className="min-h-screen bg-[#08111f] px-4 py-8 text-white sm:px-6 sm:py-12"><article className="mx-auto max-w-6xl">
    ...
    <section className="mt-8 rounded-lg border border-amber-300/25 bg-amber-300/[.07] p-5"><h2 className="text-xl font-black text-amber-100">Types of tautomerism listed on this page</h2><dl className="mt-4 grid gap-4 md:grid-cols-2"><div><dt className="font-black text-amber-50">a. Cationotropy</dt><dd className="mt-1 text-sm leading-6 text-slate-300">Tautomerism involving migration of a cation, commonly H⁺, together with the corresponding bond reorganisation.</dd></div><div><dt className="font-black text-amber-50">b. Anionotropy</dt><dd className="mt-1 text-sm leading-6 text-slate-300">Tautomerism involving migration of an anion together with the corresponding bond reorganisation.</dd></div></dl></section>
    <aside className="mt-6 rounded-lg border border-violet-300/20 bg-violet-300/[.06] p-5 text-sm leading-7 text-violet-50"><b>Examination point:</b> named keto/hydroxy and thiol/thione forms cannot be ranked from a generic “keto wins” rule. Use the particular heterocycle and the stated medium.</aside>
