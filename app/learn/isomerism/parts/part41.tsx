...
export default function IsomerismPart41() {
  ...
  return <main className="min-h-screen bg-[#08111f] px-4 py-8 text-white sm:px-6 sm:py-12"><article className="mx-auto max-w-6xl"><Link href="/learn/isomerism" className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-100">Isomerism</Link><header className="mt-6 rounded-3xl border border-cyan-300/15 bg-[radial-gradient(circle_at_top_right,_rgba(34,211,238,.18),_transparent_35%),linear-gradient(135deg,rgba(34,211,238,.08),rgba(2,6,23,.7),rgba(139,92,246,.1))] p-6 sm:p-9"><p className="text-xs font-black uppercase tracking-[.24em] text-cyan-200">Optical Isomerism source · page 112 · Part 41</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Projection and atropisomerism exercise</h1><p className="mt-3 max-w-3xl text-sm leading-7 text-[var(--text-body)]">The source continues its numbered structure-only exercise with Newman projections (90–92), Fischer projections (93–98), and three hindered biaryls (99–101). Each original structure is rendered separately rather than collapsed into a generic symmetry example.</p></header><section className="mt-7"><h2 className="text-xl font-black text-cyan-100">Newman projections (90–92)</h2><p className="mt-2 text-sm leading-7 text-[var(--text-body)]">Read the centre as the front carbon and the circle as the rear carbon. Determine chirality from the complete three-dimensional molecule, not from an isolated staggered drawing.</p><div className="mt-4 grid gap-5 md:grid-cols-3">{newmans.map((item) => <article key={item.number} className="rounded-2xl border border-white/10 bg-white/[.035] p-4"><b>{item.number}</b><NewmanCase {...item} /><p className="text-sm text-[var(--text-body)]">Keep front and rear substituent assignments fixed during any Newman-to-sawhorse conversion.</p></article>)}</div></section><section className="mt-9"><h2 className="text-xl font-black text-cyan-100">Fischer-projection set (93–98)</h2><p className="mt-2 text-sm leading-7 text-[var(--text-body)]">Horizontal bonds project forward and the vertical chain projects away. Compare the sequence of left/right hydroxyl groups and then test for an internal symmetry element; do not decide solely from the number of crossings.</p><div className="mt-4 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{fischers.map((item) => <article key={item.number} className="rounded-2xl border border-white/10 bg-white/[.035] p-4"><b>{item.number}</b><Fischer item={item} /><p className="text-sm text-[var(--text-body)]">Assign each stereocentre in the displayed orientation before pairing a structure with its mirror image.</p></article>)}</div></section><section className="mt-9"><h2 className="text-xl font-black text-cyan-100">Hindered biaryls (99–101)</h2><p className="mt-2 text-sm leading-7 text-[var(--text-body)]">The ortho halogens restrict rotation around the biaryl bond. First check whether the two aromatic-ring paths at each end are constitutionally different; only then can the twisted axis be stereogenic.</p><div className="mt-4 grid gap-5 md:grid-cols-3">{([99,100,101] as const).map((number) => <article key={number} className="rounded-2xl border border-white/10 bg-white/[.035] p-4"><b>{number}</b><Atropisomer number={number} /><p className="text-sm text-[var(--text-body)]">Examination point: restricted rotation alone is insufficient; the two paths at both termini of the axis must be distinguishable.</p></article>)}</div></section><nav className="mt-8 flex justify-between border-t border-white/10 pt-6"><Link href="/learn/isomerism/40" className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-[var(--foreground)]">← 40</Link><Link href="/learn/isomerism" className="rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-bold text-cyan-100">Chapter overview</Link></nav></article></main>;
}
