...
function CardPalette({ cards, current, progress, sessionResults, onJump, onClose }: { cards: RecallCard[]; current: number; progress: SavedProgress; sessionResults: Record<string, CardResult>; onJump: (index: number) => void; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-black/65 p-0 sm:items-center sm:justify-center sm:p-6" onMouseDown={(event: ReactMouseEvent<HTMLDivElement>) => { if (event.currentTarget === event.target) onClose(); }}>
      <div className="max-h-[82vh] w-full overflow-y-auto rounded-t-3xl border border-white/10 bg-[var(--surface)] p-5 shadow-2xl sm:max-w-xl sm:rounded-lg">
  ...
  if (sessionCards.length > 0) {
    if (done) {
      return (
        <div className="space-y-5">
          ...
          <section className="rounded-lg border border-cyan-400/20 bg-gradient-to-br from-cyan-500/12 to-violet-500/10 p-7 text-center">
    ...
    return (
      <div className="space-y-4">
        ...
        <button type="button" onClick={() => { if (!flipped) setFlipped(true); }} className="block min-h-[360px] w-full rounded-lg border border-white/[0.1] bg-[var(--surface)] p-6 text-left shadow-2xl shadow-black/20 transition hover:border-cyan-400/25 sm:min-h-[430px] sm:p-10">
      ...
      {search.trim() ? (
        ...
        <div className="grid gap-3 md:grid-cols-2">
          {RECALL_DECKS.map((deck) => {
            ...
            return (
              <button type="button" key={deck.key} onClick={() => startDeck(deck.key)} disabled={visible.length === 0} className="rounded-lg border border-white/[0.08] bg-[var(--surface)] p-5 text-left transition hover:-translate-y-0.5 hover:border-cyan-400/30 disabled:cursor-not-allowed disabled:opacity-45">
