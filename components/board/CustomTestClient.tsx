  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-lg border border-white/10 bg-white/[0.03] p-5 space-y-4">
          ...
          <div className="flex flex-wrap gap-1.5">
            {chapters.map((c) => (
              <button key={c.id} onClick={() => toggle(c.id)}
                className={`rounded-lg border px-2.5 py-1 text-xs font-semibold transition ${selected.has(c.id) ? "border-[var(--accent)]/50 bg-[var(--accent)]/15 text-white" : "border-white/10 bg-white/[0.03] text-white/55 hover:text-white/85"}`}>
                {c.title}
              </button>
            ))}
          </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
          <label className="flex items-center gap-2 text-sm text-white/70">Objective Qs
            <input type="number" min={1} max={30} value={count} onChange={(e) => setCount(Math.min(30, Math.max(1, Number(e.target.value) || 1)))}
              className="w-16 rounded-lg border border-[var(--border)] bg-[var(--surface-2)] px-2 py-1 text-white/90" />
          </label>
          <div className="flex items-center gap-1.5">
            {DIFFICULTIES.map((d) => (
              <button key={d} onClick={() => setDifficulty(d)} className={`rounded-lg border px-2.5 py-1 text-xs font-semibold ${difficulty === d ? "border-[var(--accent)]/50 bg-[var(--accent)]/15 text-white" : "border-white/10 bg-white/[0.03] text-white/60"}`}>{d}</button>
            ))}
          </div>
        ...
        <button onClick={generate} className="rounded-lg bg-[var(--accent)] px-5 py-2.5 text-sm font-black text-[var(--background)]">Generate paper →</button>
      ...
      {paper && (
        <>
          ...
          <div className="sticky top-2 z-10 flex flex-wrap items-center justify-between gap-3 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-4">
            <div className="flex gap-5 text-sm">
              <div><span className="text-2xl font-black text-white">{paper.length}</span><span className="ml-1 text-white/50">Qs</span></div>
              <div><span className="text-2xl font-black text-[var(--accent)]">{score.answered}</span><span className="ml-1 text-white/50">answered</span></div>
              <div><span className="text-2xl font-black text-[var(--text-muted)]">{score.correct}</span><span className="ml-1 text-white/50">correct</span></div>
            ...
            <div className="space-y-3">
              ...
              {subQs.map((s, i) => (
                <div key={i} className="rounded-lg border border-white/10 bg-white/[0.02] p-4">
                  <div className="mb-1 text-xs font-bold uppercase tracking-wide text-[var(--text-muted)]">{s.label}</div>
