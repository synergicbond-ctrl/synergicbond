  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center justify-between text-[11px] text-white/40">
        ...
        <span className="font-bold text-[var(--accent)]">{percentage}%</span>
      </div>
    </div>
  );
}
...
function CardPalette({ cards, current, progress, sessionResults, onJump, onClose }: { cards: RecallCard[]; current: number; progress: SavedProgress; sessionResults: Record<string, CardResult>; onJump: (index: number) => void; onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-black/65 p-0 sm:items-center sm:justify-center sm:p-6" onMouseDown={(event: ReactMouseEvent<HTMLDivElement>) => { if (event.currentTarget === event.target) onClose(); }}>
      <div className="max-h-[82vh] w-full overflow-y-auto rounded-t-lg border border-white/10 bg-[var(--surface)] p-5 sm:max-w-xl sm:rounded-lg">
        ...
        <div className="mt-4 grid grid-cols-6 gap-2 sm:grid-cols-10">
          {cards.map((card, index) => {
            ...
            const tone = index === current ? "border-[var(--accent)] bg-[var(--accent)]/20 text-white" : status === "got" ? "border-[var(--accent)]/45 bg-[var(--accent)]/10 text-[var(--accent)]" : status === "practice" ? "border-[var(--border)] bg-[var(--surface-2)] text-[var(--text-muted)]" : "border-white/10 bg-white/[0.03] text-white/45";
...
export default function RecallDecksExperience() {
    ...
    if (done) {
      return (
        <div className="space-y-5">
          ...
          <section className="rounded-lg border border-[var(--border)] bg-[var(--surface-2)] p-7 text-center">
            <p className="text-xs font-bold uppercase tracking-[0.18em] text-[var(--text-muted)]">Deck complete</p>
            ...
            <div className="mt-6 flex flex-wrap justify-center gap-2">
              {missed.length > 0 && <button type="button" onClick={() => startCards(missed)} className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-black text-[var(--background)]">Practise missed ({missed.length})</button>}
              <button type="button" onClick={() => startCards(sessionCards)} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-2 text-sm font-bold text-[var(--text-muted)]">Restart deck</button>
              <button type="button" onClick={() => { setSessionCards([]); setDone(false); }} className="rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-black text-[var(--background)]">All decks</button>
            </div>
          </section>
        </div>
      );
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <label className="flex items-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-bold text-[var(--text-muted)]"><span>⚡ Rapid mode</span><input type="checkbox" checked={rapidMode} onChange={(event: ChangeEvent<HTMLInputElement>) => { setRapidMode(event.target.checked); if (event.target.checked) setFlipped(true); }} className="accent-[#e3b341]" /></label>
            <button type="button" onClick={() => setPaletteOpen(true)} className="rounded-lg border border-[var(--border)] bg-[var(--surface)] px-3 py-2 text-xs font-bold text-[var(--text-muted)]">All cards</button>
          </div>
        </div>

        <div className="rounded-lg border border-white/[0.08] bg-[var(--surface)] p-4">
          <div className="flex items-center justify-between text-xs text-white/45"><span>Got: <strong className="text-[var(--accent)]">{gotThisSession}</strong> · Practice: <strong className="text-[var(--text-muted)]">{practiseThisSession}</strong></span><span>{current + 1}/{sessionCards.length}</span></div>
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/[0.07]"><div className="h-full rounded-full bg-[var(--accent)]" style={{ width: `${percentage}%` }} /></div>
        </div>

        <button type="button" onClick={() => { if (!flipped) setFlipped(true); }} className="block min-h-[360px] w-full rounded-lg border border-white/[0.1] bg-[var(--surface)] p-6 text-left transition hover:border-[var(--accent)]/25 sm:min-h-[430px] sm:p-10">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="rounded-full border border-[var(--border)] bg-[var(--surface-2)] px-3 py-1 text-[11px] font-bold text-[var(--text-muted)]">{RECALL_DECKS.find((deck) => deck.key === card.deck)?.title}</span>
        ...
        {!flipped ? (
          <button type="button" onClick={() => setFlipped(true)} className="w-full rounded-lg bg-[var(--accent)] px-5 py-3 text-sm font-black text-[var(--background)]">Reveal answer</button>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={() => grade("practice")} className="rounded-lg border border-[var(--border)] bg-[var(--surface-2)] px-5 py-3 text-sm font-black text-[var(--text-muted)]">Need practice</button>
            <button type="button" onClick={() => grade("got")} className="rounded-lg border border-[var(--accent)]/30 bg-[var(--accent)]/10 px-5 py-3 text-sm font-black text-[var(--accent)]">Got it ✓</button>
          </div>
        )}
  ...
  return (
    <div className="space-y-6">
      <header>
        <p className="text-xs font-bold uppercase tracking-[0.22em] text-[var(--text-muted)]">Revision</p>
      ...
      <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
        <label className="relative"><span className="pointer-events-none absolute left-4 top-3.5 text-white/30">⌕</span><input value={search} onChange={(event: ChangeEvent<HTMLInputElement>) => setSearch(event.target.value)} placeholder="Search cards, chapters, reagents or reactions…" className="w-full rounded-lg border border-[var(--border)] bg-[var(--surface)] py-3 pl-10 pr-4 text-sm text-white outline-none placeholder:text-white/25 focus:border-[var(--accent)]/50" /></label>
        <label className="flex items-center justify-center gap-2 rounded-lg border border-[var(--border)] bg-[var(--surface)] px-4 py-3 text-sm font-bold text-[var(--text-muted)]"><input type="checkbox" checked={importantOnly} onChange={(event: ChangeEvent<HTMLInputElement>) => setImportantOnly(event.target.checked)} className="accent-[#e3b341]" /> Important only</label>
      </div>
      ...
      {search.trim() ? (
        <section>
          ...
          {searchMatches.length === 0 ? <div className="rounded-lg border border-dashed border-white/10 p-8 text-center text-sm text-white/45">No matching recall cards.</div> : (
            <div className="grid gap-2 md:grid-cols-2">
              {searchMatches.map((card) => <button type="button" key={card.id} onClick={() => startCards([card, ...ALL_RECALL_CARDS.filter((item) => item.deck === card.deck && item.id !== card.id)])} className="rounded-lg border border-white/[0.08] bg-[var(--surface)] p-4 text-left transition hover:border-[var(--accent)]/30"><p className="text-xs font-bold text-[var(--accent)]">{card.chapter}</p><div className="mt-2 line-clamp-2 text-sm font-semibold leading-6 text-white/75">{renderChemistry(card.front)}</div></button>)}
            </div>
          )}
        </section>
      ) : (
        <div className="grid gap-3 md:grid-cols-2">
          {RECALL_DECKS.map((deck) => {
            ...
            return (
              <button type="button" key={deck.key} onClick={() => startDeck(deck.key)} disabled={visible.length === 0} className="rounded-lg border border-white/[0.08] bg-[var(--surface)] p-5 text-left transition hover:-translate-y-0.5 hover:border-[var(--accent)]/30 disabled:cursor-not-allowed disabled:opacity-45">
                <div className="flex items-start justify-between gap-3"><div className="flex items-start gap-3"><span className="flex h-10 w-10 items-center justify-center rounded-lg border border-white/10 bg-white/[0.04] text-lg text-[var(--accent)]">{deck.icon}</span><div><h2 className="font-black text-white">{deck.title}</h2><p className="mt-1 text-xs leading-5 text-white/45">{deck.description}</p></div></div><span className="shrink-0 rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-1 text-[11px] font-bold text-white/50">{visible.length} cards</span></div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-white/[0.08] bg-white/[0.02] p-4 text-sm text-white/45">
        <span>{ALL_RECALL_CARDS.length} source-derived recall cards available.</span>
        <div className="flex gap-3"><Link href="/revision" className="font-bold text-[var(--accent)]">Revision Queue</Link><button type="button" onClick={() => { if (window.confirm("Clear all saved recall progress on this device?")) { saveProgress({}); setProgress({}); } }} className="font-semibold text-white/40 hover:text-[var(--text-muted)]">Clear progress</button></div>
      </div>
    </div>
  );
}
