    ...
    <div className="grid gap-4 lg:grid-cols-3">
      {statements.map((statement) => (
        <article key={statement.title} className={`rounded-3xl border p-5 ${toneMap[statement.tone]}`}>
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--foreground)]">{statement.text}</p>
        </article>
      ))}
    </div>
  );
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      {trendCards.map((item) => (
        <article key={item.index} className={`rounded-3xl border p-5 ${toneMap[item.tone]}`}>
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--foreground)]">{item.explanation}</p>
        ...
        <tbody className="divide-y divide-white/10 bg-[var(--background)]/70">
          {rows.map(([comparison, trend, basis]) => (
            <tr key={comparison}>
              ...
              <td className="px-4 py-3 text-[var(--text-body)]">{basis}</td>
            </tr>
          ))}
        </tbody>
      </table>
