...
export function LearningObjectives({ items }: { items: string[] }) {
  return <section aria-labelledby="objectives" className="rounded-2xl border border-cyan-300/20 bg-cyan-300/[0.06] p-5"><p className="text-xs font-black uppercase tracking-[0.18em] text-cyan-200">Learning objectives</p><h2 id="objectives" className="mt-1 text-lg font-bold text-white">By the end, you can</h2><ul className="mt-3 grid gap-2 text-sm text-[var(--foreground)] sm:grid-cols-2">{items.map((item) => <li key={item} className="flex gap-2"><span aria-hidden="true" className="text-cyan-300">◆</span>{item}</li>)}</ul></section>;
}

export function ConceptCard({ title, children }: { title: string; children: ReactNode }) {
  return <section className="rounded-2xl border border-sky-300/20 bg-[var(--background)]/45 p-5 shadow-lg shadow-black/10"><h2 className="text-lg font-black text-sky-200">{title}</h2><div className="mt-3 space-y-3 text-sm leading-relaxed text-[var(--foreground)] sm:text-base">{children}</div></section>;
}
...
export function ComparisonTable({ headers, rows }: { headers: ReactNode[]; rows: ReactNode[][] }) {
  return <section aria-label="Comparison" className="overflow-x-auto rounded-2xl border border-white/10 bg-[var(--background)]/45"><table className="min-w-[38rem] w-full text-left text-sm sm:text-base"><thead className="bg-cyan-300/[0.08] text-cyan-100"><tr>{headers.map((header) => <th key={String(header)} scope="col" className="px-4 py-3 font-black">{header}</th>)}</tr></thead><tbody className="divide-y divide-white/10 text-[var(--foreground)]">{rows.map((row, rowIndex) => <tr key={rowIndex}>{row.map((cell, cellIndex) => <td key={cellIndex} className="px-4 py-4 align-top leading-relaxed">{cell}</td>)}</tr>)}</tbody></table></section>;
}
...
export function FigureCaption({ children }: { children: ReactNode }) {
  return <p className="mt-3 border-t border-white/10 pt-3 text-sm leading-relaxed text-[var(--text-body)]">{children}</p>;
}
