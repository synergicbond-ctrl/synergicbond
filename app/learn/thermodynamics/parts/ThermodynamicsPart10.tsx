  ...
  return (
    <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#07111f] p-3 md:p-5">
      ...
      >
        ...
        <rect x="80" y="100" width="1040" height="505" rx="36" fill="#0f172a" stroke="#f59e0b" strokeWidth="4" />
        ...
        <line x1="500" y1="165" x2="500" y2="335" stroke="#67e8f9" strokeWidth="5" strokeLinecap="round">
          <animate
            attributeName="y2"
            values="365;235;365"
            dur="5s"
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      {rows.map((row) => (
        <article key={row.gas} className={`rounded-3xl border p-5 ${toneMap[row.tone]}`}>
          ...
          <p className="mt-3 text-xs leading-5 text-[var(--text-body)]">{row.caution}</p>
        </article>
      ))}
    </div>
  );
  ...
  return (
    <article className="rounded-[2rem] border border-amber-300/25 bg-amber-300/[0.06] p-5 md:p-7">
      ...
      <div className="mt-5 rounded-2xl border border-rose-300/20 bg-rose-300/[0.05] p-5">
        ...
        <p className="mt-4 rounded-xl border border-amber-300/20 bg-amber-300/[0.06] p-4 text-sm leading-6 text-[var(--foreground)]">
        ...
        <tbody className="divide-y divide-white/10 bg-[var(--background)]/70">
          {rows.map(([process, heat, work, temp, note]) => (
            <tr key={process}>
              ...
              <td className="px-4 py-3 text-[var(--text-body)]">{temp}</td>
              <td className="px-4 py-3 text-[var(--text-body)]">{note}</td>
            </tr>
          ))}
        </tbody>
      </table>
