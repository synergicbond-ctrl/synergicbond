...
export default function QuizAndFeedback({ questions, chapterId }: { questions: Question[]; chapterId: string }) {
  ...
  if (questions.length === 0) {
    return (
      <div className="p-8 bg-white rounded-2xl border border-[var(--border)] text-center text-slate-500 font-medium">
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-sm border border-[var(--border)]/80 max-w-2xl mx-auto space-y-6">
      ...
      <div className="space-y-3">
        {currentQ.options.map((opt) => {
          let optStyle = "bg-slate-50 border-[var(--border)] hover:border-indigo-500 text-slate-700";
          ...
          if (isAnswered) {
            if (opt.startsWith(currentQ.answer)) {
              ...
              optStyle = "bg-slate-50 border-[var(--border)] opacity-60 text-slate-500";
            }
      ...
      {!isAnswered ? (
        ...
        <div className="space-y-4 pt-4 border-t border-[var(--border)] animate-fadeIn">
          ...
          {/* Solution Breakdown */}
          <div className="p-4 rounded-xl bg-slate-50 border border-[var(--border)]/60 text-slate-700 text-sm space-y-1">
          ...
          <button 
            onClick={handleNext}
            className="w-full py-3.5 bg-[var(--surface-2)] hover:bg-[var(--surface)] text-white font-bold rounded-xl transition"
          >
