...
function MathContent({ children, className = "" }: { children: string; className?: string }) {
  return (
    <div className={`text-[var(--text-body)] ${className}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          li: ({ children: value }) => <li className="text-[var(--text-body)]">{value}</li>,
        }}
      >
        {children}
      </ReactMarkdown>
...
function ParticleGrid({ stateType }: { stateType: "solid" | "liquid" | "gas" }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 rounded-xl border border-[var(--border-strong)] bg-[var(--surface)]/50">
      <svg viewBox="0 0 130 130" className="w-32 h-32 rounded-xl bg-[var(--background)]/70 shadow-inner">
        {dots.map(([x, y], i) => (
...
function VolumeEquivalence() {
  return (
    <article className="rounded-2xl border border-[var(--border-strong)] bg-[var(--background)]/70 p-6 shadow-xl">
...
function InfoCard({ title, children, accent = "cyan" }: { title: string; children: ReactNode; accent?: "cyan" | "amber" | "purple" }) {
  ...
  return (
    <aside className={`rounded-2xl border p-5 shadow-2xl shadow-black/10 ${accents[accent]}`}>
      ...
      <div className="mt-3 text-sm leading-7 text-[var(--text-body)] sm:text-base">{children}</div>
    </aside>
  );
}
...
function DataTable({ caption, columns, rows }: { caption: string; columns: string[]; rows: Array<Array<ReactNode>> }) {
  return (
    <div className="my-6 overflow-x-auto rounded-2xl border border-[var(--border-strong)]/80 bg-[var(--background)]/65 shadow-2xl shadow-black/20">
      <table className="w-full min-w-[760px] border-collapse text-left">
        <caption className="border-b border-[var(--border-strong)]/70 bg-gradient-to-r from-cyan-400/10 via-slate-900 to-purple-400/10 px-5 py-4 text-left text-lg font-black text-white">
          {caption}
        </caption>
        <thead>
          <tr className="bg-[var(--surface)]/90">
            {columns.map((column) => (
              <th key={column} className="border-b border-[var(--border-strong)]/70 px-4 py-3 text-xs font-black uppercase tracking-[0.16em] text-cyan-300">
                {column}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr key={`row-${rowIndex}`} className="border-b border-[var(--border)]/80 last:border-b-0 hover:bg-[var(--surface-2)]/30 transition-colors">
              {row.map((cell, cellIndex) => (
                cellIndex === 0 
                  ? <th key={cellIndex} className="bg-[var(--surface)]/55 px-4 py-4 font-bold text-slate-100">{cell}</th>
                  : <td key={cellIndex} className="px-4 py-4 leading-7 text-[var(--text-body)]">{cell}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
function ExemplarProblem({ problem }: { problem: ProblemData }) {
  return (
    <details className="group overflow-hidden rounded-2xl border border-[var(--border-strong)]/80 bg-gradient-to-br from-slate-900/95 to-slate-950/95 shadow-xl shadow-black/20 transition hover:border-cyan-400/35">
      <summary className="flex items-start gap-4 border-b border-[var(--border)]/90 p-5 cursor-pointer outline-none focus:ring-2 focus:ring-cyan-400/50">
      ...
      <div className="p-5 bg-[var(--surface)]/40">
        {problem.options.length > 0 && (
          <ol className="grid gap-2 mb-5 sm:grid-cols-2">
            {problem.options.map((option, index) => (
              <li key={index} className="flex gap-3 rounded-xl border border-[var(--border)] bg-[var(--background)]/60 p-3 text-sm text-[var(--text-body)]">
        ...
        <div className="border-t border-[var(--border-strong)]/80 pt-5">
  ...
  return (
    <div className="mt-10">
      <div className="mb-4 flex items-end justify-between gap-4">
        ...
        <span className="rounded-full border border-[var(--border-strong)] bg-[var(--surface-2)]/50 px-3 py-1 text-xs font-bold text-[var(--text-muted)]">
...
function TransformationCards() {
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {cards.map((card) => (
        <article key={card.title} className="rounded-2xl border border-[var(--border-strong)] bg-[var(--surface)]/70 p-5 shadow-lg">
    ...
    <ol className="relative space-y-4 before:absolute before:bottom-6 before:left-6 before:top-6 before:w-px before:bg-gradient-to-b before:from-cyan-400 before:via-purple-400 before:to-amber-400">
      {items.map(([year, title, text], index) => (
        <li key={title} className="relative grid grid-cols-[3rem_1fr] gap-4">
          <span className="z-10 grid h-12 w-12 place-items-center rounded-xl border border-cyan-400/30 bg-[var(--background)] text-xs font-black text-cyan-300">
            {String(index + 1).padStart(2, "0")}
          </span>
          <div className="rounded-2xl border border-[var(--border-strong)]/80 bg-[var(--surface)]/65 p-4 shadow-md">
...
function MatterTree() {
  return (
    <div className="my-6 rounded-2xl border border-[var(--border-strong)] bg-[var(--background)]/65 p-5 shadow-lg">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-cyan-400/20 bg-[var(--surface)]/80 p-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <span className="rounded-lg bg-[var(--surface-2)] border border-[var(--border-strong)] p-3 text-center text-sm font-bold text-[var(--foreground)]">Homogeneous</span>
            <span className="rounded-lg bg-[var(--surface-2)] border border-[var(--border-strong)] p-3 text-center text-sm font-bold text-[var(--foreground)]">Heterogeneous</span>
          </div>
        </div>
        <div className="rounded-xl border border-purple-400/20 bg-[var(--surface)]/80 p-4">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3">
            <span className="rounded-lg bg-[var(--surface-2)] border border-[var(--border-strong)] p-3 text-center text-sm font-bold text-[var(--foreground)]">Elements</span>
            <span className="rounded-lg bg-[var(--surface-2)] border border-[var(--border-strong)] p-3 text-center text-sm font-bold text-[var(--foreground)]">Compounds</span>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
      {laws.map(([title,note,example],index)=>(
        <article key={title} className="flex min-h-48 flex-col rounded-2xl border border-[var(--border-strong)] bg-gradient-to-br from-slate-900 to-slate-950 p-5 shadow-lg">
...
function MoleBridge() {
  return (
    <div className="grid items-center gap-4 rounded-2xl border border-[var(--border-strong)] bg-[var(--background)]/70 p-5 text-center md:grid-cols-[1fr_auto_1fr_auto_1fr] shadow-lg">
      {[
...
function LimitingReagentVisual() {
  return (
    <div className="grid gap-5 rounded-2xl border border-[var(--border-strong)] bg-[var(--background)]/70 p-5 lg:grid-cols-[1fr_auto_1fr] shadow-lg mt-6">
      <div>
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
      {cards.map(([title,formula,note])=>(
        <article key={title} className="rounded-2xl border border-[var(--border-strong)] bg-gradient-to-br from-slate-900 to-slate-950 p-5 shadow-lg">
        ...
        {items.map((item) => (
          <article key={item.title} className={`rounded-2xl border p-5 shadow-lg ${accentClasses[item.accent]}`}>
            <div className="flex items-center gap-3">
              <span className="grid h-12 w-12 place-items-center rounded-xl border border-current/25 bg-[var(--background)]/55 text-lg font-black">{item.icon}</span>
              <h4 className="text-lg font-black text-white">{item.title}</h4>
            </div>
            <ul className="mt-4 space-y-3">
              {item.points.map((point) => (
                <li key={point} className="flex gap-3 leading-7 text-[var(--text-body)]">
...
function PhaseTransitionMap() {
  ...
  return (
    <section className="my-8 rounded-2xl border border-[var(--border-strong)] bg-[var(--background)]/70 p-5 shadow-xl">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        {transitions.map(([from, to, forward, reverse]) => (
          <article key={forward} className="rounded-xl border border-[var(--border-strong)] bg-[var(--surface)]/75 p-4">
            <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3 text-center">
              <span className="rounded-xl bg-[var(--surface-2)] p-4 font-black text-cyan-200">{from}</span>
              ...
              <span className="rounded-xl bg-[var(--surface-2)] p-4 font-black text-amber-200">{to}</span>
            </div>
          </article>
        ))}
      </div>
    ...
    <section className="mt-10">
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        {examples.map((example, exampleIndex) => (
          <article key={example.title} className="rounded-2xl border border-[var(--border-strong)] bg-[var(--surface)]/75 p-5 shadow-lg">
    ...
    <section className="mt-10 rounded-2xl border border-purple-400/25 bg-gradient-to-br from-purple-400/[0.07] via-slate-950 to-cyan-400/[0.05] p-6 shadow-xl">
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        {postulates.map((postulate, index) => (
          <article key={postulate} className="grid grid-cols-[3rem_1fr] gap-4 rounded-xl border border-[var(--border-strong)] bg-[var(--surface)]/70 p-4">
            ...
            <p className="leading-7 text-[var(--text-body)]">{postulate}</p>
          </article>
        ))}
      </div>
...
function MassLanguagePanel() {
  return (
    <section className="mt-10">
      ...
      <div className="mt-4 rounded-2xl border border-[var(--border-strong)] bg-[var(--background)]/70 p-5">
    ...
    <section className="mt-10">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-7">
        {steps.map(([number, title, note]) => (
          <article key={number} className="rounded-xl border border-[var(--border-strong)] bg-[var(--surface)]/75 p-4">
    ...
    <section className="mt-10 rounded-2xl border border-amber-400/25 bg-amber-400/[0.035] p-6 shadow-xl">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        {stages.map(([equation, note], index) => (
          <article key={equation} className="rounded-xl border border-[var(--border-strong)] bg-[var(--background)]/75 p-5">
...
export default function ChemistryDashboard() {
  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-[var(--background)] text-[var(--foreground)] font-sans selection:bg-cyan-400/30">
      {/* Background Decor */}
      ...
      <header className="relative z-10 mx-auto grid min-h-[88vh] w-full max-w-[1440px] items-center gap-10 px-4 py-16 lg:grid-cols-[1.05fr_.95fr]">
        <div>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            <a href="#answer-key" className="rounded-xl border border-slate-600 bg-[var(--surface)]/70 px-6 py-3.5 font-black text-white no-underline transition hover:bg-[var(--surface-2)] hover:border-slate-500">Search 50 Questions</a>
          </div>
      ...
      <section id="measurement" className="relative z-10 mx-auto w-full max-w-[1440px] scroll-mt-20 px-4 py-20">
        ...
        <div className="mt-12">
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            <article className="rounded-2xl border border-[var(--border-strong)] bg-[var(--surface)]/70 p-6 text-center shadow-lg">
      ...
      <section id="mole" className="relative z-10 mx-auto w-full max-w-[1440px] scroll-mt-20 px-4 py-20">
        ...
        <div className="my-8 rounded-2xl bg-gradient-to-r from-cyan-400 via-purple-400 to-amber-400 p-6 text-center shadow-lg">
          <div className="bg-[var(--background)] rounded-xl p-4 inline-block">
      ...
      <section id="solutions" className="relative z-10 mx-auto w-full max-w-[1440px] scroll-mt-20 px-4 py-20">
        ...
        <div className="mt-6">
          <InfoCard title="Dilution equation" accent="purple">
            ...
            <p className="mt-2 text-[var(--text-body)]">The moles of solute remain constant while solvent changes the total volume.</p>
          </InfoCard>
        </div>
      ...
      <section id="answer-key" className="relative z-20 border-t-4 border-cyan-400/50 bg-[var(--surface)] px-4 py-24 shadow-[0_-20px_50px_rgba(0,0,0,0.5)]">
        <div className="mx-auto w-full max-w-[1440px]">
          ...
          <div className="sticky top-20 z-30 mb-8 rounded-2xl border border-cyan-400/40 bg-[var(--background)]/80 p-4 backdrop-blur-xl shadow-2xl">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full rounded-xl border border-[var(--border-strong)] bg-[var(--surface)] py-4 pl-12 pr-4 text-lg text-white placeholder-slate-500 focus:border-cyan-400 focus:outline-none focus:ring-2 focus:ring-cyan-400/50 transition-all"
              />
              {searchQuery && (
                <button 
          ...
          {filteredQuestions.length === 0 ? (
            <div className="py-20 text-center text-slate-500 border-2 border-dashed border-[var(--border-strong)] rounded-2xl">
      ...
      <footer className="border-t border-[var(--border)] bg-[var(--background)] px-4 py-12 text-center text-slate-500">
