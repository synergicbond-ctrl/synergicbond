...
export async function generateMetadata({
  ...
  return part
    ? {
        ...
        description: `Complete JEE Advanced s-block notes, Part ${part.number}: ${part.focus}`,
      }
    : {};
}

export default async function SBlockPartPage({
  params,
}: {
  params: Promise<{ part: string }>;
}) {
  ...
  return (
    ...
    >
      ...
      <article className="mx-auto w-full max-w-6xl space-y-6 text-white">
        <header className="overflow-hidden rounded-2xl border border-[#24405c] border-l-4 border-l-cyan-300 bg-[radial-gradient(circle_at_top_right,rgba(139,92,246,.18),transparent_38%),#122232] px-5 py-5 sm:px-7">
          <p className="text-[11.5px] font-black uppercase tracking-[.14em] text-cyan-300">
            S-block Elements · Part {part.number} · {sectionRangeLabel(part)}
          </p>

          <h1 className="mt-2 font-serif text-3xl font-black leading-tight text-[#eef3f8] sm:text-4xl">
            {part.title}
          </h1>
