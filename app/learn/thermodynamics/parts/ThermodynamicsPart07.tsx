        ...
        <article key={item.form} className={`rounded-3xl border p-5 ${toneMap[item.tone]}`}>
          ...
          <div className="mt-4 grid gap-2 text-sm">
            <div className="rounded-xl bg-black/25 p-3">
              ...
              <span className="ml-2 text-[var(--foreground)]">{item.intensity}</span>
            </div>
            <div className="rounded-xl bg-black/25 p-3">
              <span className="font-black text-white">Capacity factor:</span>
              <span className="ml-2 text-[var(--foreground)]">{item.capacity}</span>
            </div>
          </div>
          <div className="mt-3 rounded-xl bg-black/25 p-3 text-center font-mono font-black text-white">
            {item.differential}
          </div>
          <p className="mt-3 text-sm leading-6 text-[var(--text-body)]">{item.example}</p>
        </article>
      ))}
    </div>
  );
        ...
        <tbody className="divide-y divide-white/10 bg-[var(--background)]/70">
          {signRows.map(([name, relation, dv, work, meaning]) => (
            <tr key={name}>
              ...
              <td className="px-4 py-3 text-[var(--text-body)]">{relation}</td>
              <td className="px-4 py-3 font-mono text-[var(--foreground)]">{dv}</td>
              <td className="px-4 py-3 font-mono font-black text-cyan-200">{work}</td>
              <td className="px-4 py-3 leading-6 text-[var(--text-body)]">{meaning}</td>
            </tr>
          ))}
        </tbody>
      </table>
        ...
        <tbody className="divide-y divide-white/10 bg-[var(--background)]/70">
          {unitRows.map(([unit, logic, value]) => (
            <tr key={unit}>
              ...
              <td className="px-4 py-3 font-mono text-[var(--text-body)]">{logic}</td>
