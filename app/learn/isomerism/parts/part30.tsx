...
function Card({ children, n, active, note }: { children: React.ReactNode; n: number; active: boolean; note: string }) { return <article className="rounded-lg border border-white/10 bg-white/[.035] p-3"><div className="flex items-start justify-between gap-2"><span className="rounded bg-slate-700 px-2 py-1 text-xs font-black">{n}</span><span className={active ? "rounded-full bg-emerald-400/15 px-2 py-1 text-xs font-black text-emerald-200" : "rounded-full bg-rose-400/15 px-2 py-1 text-xs font-black text-rose-200"}>{active ? "optically active" : "inactive"}</span></div>{children}<p className="mt-2 border-t border-white/10 pt-2 text-xs leading-5 text-slate-300">{note}</p></article> }
...
export default function IsomerismPart30() {
  return <main className="min-h-screen bg-[#08111f] px-4 py-8 text-white sm:px-6 sm:py-12"><article className="mx-auto max-w-7xl">
    ...
    <aside className="mt-6 rounded-lg border border-amber-300/25 bg-amber-300/[.07] p-5 text-sm leading-7 text-amber-50"><strong>Examination point:</strong> bulky ortho groups alone are not a sufficient test. Check both persistence of the restricted conformation and the absence of a plane, centre or other improper symmetry element.</aside>
