...
export default function EquivalentWeightHubPage() {
  ...
  return (
    <ChemistryNotesShell>
      ...
      <footer className="flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-slate-800 bg-slate-900/40 p-6 backdrop-blur-sm">
        <button
        ...
        {activeSection < SECTIONS.length ? (
          ...
          <Link
            ...
            className="rounded-xl bg-purple-600 px-6 py-2.5 text-sm font-extrabold text-white shadow-lg hover:bg-purple-700 border border-purple-500/20 transition-all text-center"
          >
            Proceed to Titration & Volumetric Analysis →
          </Link>
        )}
      </footer>
    </ChemistryNotesShell>
