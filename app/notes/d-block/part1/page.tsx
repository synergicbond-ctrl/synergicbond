    ...
    <main className="dblock-page">
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
        <header className="overflow-hidden rounded-[2rem] border border-cyan-400/20 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,0.18),transparent_34%),linear-gradient(135deg,#07111f,#0b1a2c_55%,#10142a)] px-6 py-9 shadow-[0_30px_100px_rgba(0,0,0,0.35)] sm:px-10 sm:py-12">
          ...
          <p className="mt-5 max-w-4xl text-lg leading-8 text-[var(--text-body)]">A rigorous foundation for deciding what belongs to the d-block, what qualifies as a transition element, and how near-degenerate ns and (n−1)d orbitals generate the configurations used throughout the chapter.</p>
        </header>

        <DBlockPartNavigation part={1} position="top" />
          ...
          {PAGES.map((page, index) => {
            ...
            return (
              <section id={page.id} key={page.id} className="scroll-mt-24 rounded-[2rem] border border-white/10 bg-[#081321] px-5 py-8 shadow-[0_22px_70px_rgba(0,0,0,0.24)] print:break-after-page sm:px-9 sm:py-10">
                ...
                <ReactMarkdown remarkPlugins={[remarkGfm, remarkMath]} rehypePlugins={[rehypeKatex]} components={createMarkdownComponents(1)}>
                  {page.markdown}
                </ReactMarkdown>
                {Visual ? <Visual /> : null}
              </section>
        ...
        <DBlockPartNavigation part={1} position="bottom" />
      </div>
    </main>
  );
}
