...
function PyridineTautomer({ kind }: { kind: Tautomer }) {
  ...
  return <figure className="rounded-2xl border border-white/10 bg-[var(--background)]/50 p-3">
    ...
    <figcaption className="text-center text-sm font-bold text-[var(--foreground)]">{labels[kind]}</figcaption>
  </figure>;
}
...
export default function IsomerismPart61() {
  return <main className="min-h-screen bg-[#08111f] px-4 py-8 text-white sm:px-6 sm:py-12"><article className="mx-auto max-w-6xl">
    ...
    <section className="mt-8 rounded-2xl border border-amber-300/25 bg-amber-300/[.07] p-5"><h2 className="text-xl font-black text-amber-100">Types of tautomerism listed on this page</h2><dl className="mt-4 grid gap-4 md:grid-cols-2"><div><dt className="font-black text-amber-50">a. Cationotropy</dt><dd className="mt-1 text-sm leading-6 text-[var(--text-body)]">Tautomerism involving migration of a cation, commonly H⁺, together with the corresponding bond reorganisation.</dd></div><div><dt className="font-black text-amber-50">b. Anionotropy</dt><dd className="mt-1 text-sm leading-6 text-[var(--text-body)]">Tautomerism involving migration of an anion together with the corresponding bond reorganisation.</dd></div></dl></section>
    ...
    <nav className="mt-8 flex justify-between border-t border-white/10 pt-6"><Link href="/learn/isomerism/60" className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-[var(--foreground)]">← 60</Link><Link href="/learn/isomerism" className="rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-bold text-cyan-100">Chapter overview</Link></nav>
  </article></main>;
}
