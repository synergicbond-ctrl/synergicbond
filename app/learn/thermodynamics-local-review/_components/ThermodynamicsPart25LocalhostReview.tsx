...
function DerivRow({ step, result }: { step: string; result: string }) {
  return (
    <div className="grid gap-2 border-b border-[var(--border)] pb-3 pt-1 md:grid-cols-[1fr_2fr]">
      <span className="text-xs font-black uppercase tracking-wider text-[var(--text-muted)]">{step}</span>
      <span className="font-mono text-sm text-[var(--foreground)]">{result}</span>
    </div>
  );
}
...
function FundamentalRelationsSVG() {
  return (
    <figure className="mx-auto w-full max-w-4xl overflow-hidden rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-3 md:p-5">
      ...
      <figcaption className="px-3 pt-3 text-center text-sm leading-6 text-[var(--text-muted)]">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {[
              ...
              <aside key={String(title)} className="rounded-lg border border-amber-400/25 bg-amber-400/[0.06] p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-[var(--text-muted)]">{String(body)}</p>
              </aside>
            ))}
          </div>
        </Section>
            ...
            {practiceProblems.map((p, i) => (
              <article key={i} className="rounded-lg border border-[var(--border)] bg-slate-900/60 p-4">
                ...
                <div className="mt-3 rounded-xl border border-emerald-300/20 bg-emerald-300/[0.06] p-3">
                  ...
                  <p className="mt-1 text-sm leading-6 text-[var(--text-body)]">{p.a}</p>
                </div>
              </article>
            ))}
          </div>
