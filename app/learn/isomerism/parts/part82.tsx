...
export default function IsomerismPart82(){return <main className="min-h-screen bg-[#08111f] px-4 py-8 text-white sm:px-6 sm:py-12"><article className="mx-auto max-w-6xl"><Link href="/learn/isomerism" className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-100">Isomerism</Link><header className="mt-6 rounded-3xl border border-cyan-300/15 bg-[#08111f] p-6 sm:p-9"><p className="text-xs font-black uppercase tracking-[.24em] text-cyan-200">Isomerism source · page 72 · Part 82</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Exercise 1: identify structures that show geometrical isomerism</h1><p className="mt-4 text-[var(--text-body)]">For every source structure, inspect each C=C independently. A molecule qualifies only if at least one double bond has two different substituents at both trigonal carbons.</p></header><section className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Array.from({length:36},(_,index)=>{const n=index+1; const valid=accepted.has(n); return <article key={n} className="rounded-2xl border border-white/10 bg-white/[.035] p-4"><h2 className="font-black text-cyan-100">Source structure {n}</h2><SourceAlkene number={n} valid={valid}/><p className="text-sm leading-6 text-[var(--text-body)]">{valid ? reasons[n] : "At least one alkene carbon has two identical substituents, or the displayed double bond is terminal; the E/Z condition fails."}</p></article>})}</section><aside className="mt-7 rounded-2xl border border-amber-300/25 bg-amber-300/[.07] p-5 text-sm leading-6 text-amber-50"><b>Answer set from the marked source page:</b> 4, 6, 9, 11, 13, 16, 23–27, 30, 31, 33–36. In a polyene, do not reject the whole molecule because a different double bond is terminal: test each π bond separately.</aside></article></main>}
