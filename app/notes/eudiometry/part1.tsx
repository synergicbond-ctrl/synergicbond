...
const ConceptCard = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div className="my-6 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6 shadow-xl">
    <h3 className="border-b border-[var(--border)] pb-3 text-lg font-bold text-[var(--accent)]">{title}</h3>
...
const FormulaBox = ({ title, formula, description }: { title: string; formula: string; description?: string }) => (
  <div className="my-6 rounded-lg border border-amber-400/30 bg-[var(--background)] p-6 shadow-lg text-center space-y-3">
...
const StepBox = ({ number, title, children }: { number: number; title: string; children: React.ReactNode }) => (
  <div className="flex gap-3 text-left">
    <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[var(--surface-2)] text-xs font-bold text-[var(--accent)]">
      {number}
    </span>
    <div>
...
const CautionBox = ({ title, children }: { title: string; children: React.ReactNode }) => (
  <div className="my-6 rounded-lg border border-amber-500/40 bg-amber-500/10 p-5 text-sm leading-relaxed text-amber-200">
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] px-4 py-8 text-[var(--foreground)] md:px-10">
      <article className="mx-auto max-w-5xl space-y-10">
        {/* Module Header */}
        <header className="rounded-lg border border-[var(--border)] bg-[var(--surface)] p-6 shadow-2xl md:p-8">
          <div className="flex items-center justify-between">
            <span className="rounded-full bg-[var(--surface-2)] px-3 py-1 text-xs font-bold uppercase tracking-[0.2em] text-[var(--accent)]">
              Module 1 of 6
            </span>
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <ConceptCard title="1. Gay-Lussac's Law of Combining Volumes">
              ...
              <div className="mt-3 rounded-lg bg-[var(--background)] p-3 border border-[var(--border)] text-center font-mono text-xs text-amber-300">
            ...
            <ConceptCard title="2. Avogadro's Volume Relation">
              ...
              <div className="mt-3 rounded-lg bg-[var(--background)] p-3 border border-[var(--border)] text-center font-mono text-xs text-emerald-300">
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <CautionBox title="Negligible Volume of Solids and Liquids">
              ...
              <p className="mt-2 text-xs text-[var(--text-muted)] font-semibold">
