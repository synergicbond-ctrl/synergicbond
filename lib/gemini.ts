...
export async function generateText(prompt: string, _usePro = false): Promise<string> {
  const response = await getAI().models.generateContent({ model: "gemini-1.5-flash", contents: prompt });
...
export async function generateJSON(prompt: string): Promise<string> {
  ...
  const response = await getAI().models.generateContent({ model: "gemini-1.5-flash", contents: fullPrompt });
  return response.text ?? "";
}
