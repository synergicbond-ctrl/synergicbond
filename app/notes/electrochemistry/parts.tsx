  ...
  return (
    <Card>
      ...
      {section.paragraphs?.map((paragraph) => (
        <p key={paragraph} className="mt-3 text-[15px] leading-7 text-[var(--foreground)]">
          {paragraph}
        </p>
      ))}
      {section.bullets ? (
        <ul className="mt-4 space-y-2 pl-5 text-[15px] leading-7 text-[var(--foreground)]">
              ...
              {section.table.rows.map((row, rowIndex) => (
                <tr key={`${row[0]}-${rowIndex}`} className="odd:bg-black/10">
                  {row.map((cell, cellIndex) => (
                    <td key={`${cell}-${cellIndex}`} className="border-b border-white/[.06] px-4 py-3 align-top leading-6 text-[var(--foreground)]">
                      {cell}
                    </td>
                  ))}
                </tr>
...
function WorkedExample({ example }: { example: ExampleSpec }) {
  return (
    <Card tone="violet">
      ...
      <p className="mt-3 text-[15px] leading-7 text-[var(--foreground)]">{example.question}</p>
      <ol className="mt-4 space-y-2 pl-5 text-[15px] leading-7 text-[var(--foreground)]">
