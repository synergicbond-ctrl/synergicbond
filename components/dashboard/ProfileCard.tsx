...
export function ProfileCard({
  ...
  return (
    <Link
      ...
      className="block rounded-lg border border-white/[0.08] bg-[var(--surface)] p-5 hover:border-cyan-500/30 transition group"
    >
      <div className="flex items-center gap-3.5">
        <div className="h-12 w-12 rounded-lg bg-[var(--surface-2)] border border-[var(--border)] flex items-center justify-center text-lg font-black text-[var(--foreground)] flex-shrink-0">
          {name.charAt(0).toUpperCase()}
        </div>
        <div className="min-w-0 flex-1">
          <h3 className="font-black text-white text-sm truncate">{name}</h3>
          <p className="text-[11px] text-[var(--text-muted)] font-bold mt-0.5">{subscriptionLabel}</p>
        </div>
        <ArrowRight className="h-4 w-4 text-white/30 group-hover:text-[var(--accent)] transition flex-shrink-0" />
      </div>
