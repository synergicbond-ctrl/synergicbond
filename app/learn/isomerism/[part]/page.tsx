...
import { IsomerismPartView, isomerismParts } from "../parts";

export function generateStaticParams() { return isomerismParts.map((part) => ({ part: String(part.number) })); }
  ...
  return (
    <nav aria-label="Isomerism lesson navigation" className="bg-[#08111f] px-4 pb-10 text-white sm:px-6">
      <div className="mx-auto flex max-w-5xl items-center justify-between gap-3 border-t border-white/10 pt-6">
        ...
        <Link href="/learn/isomerism" className="rounded-xl border border-cyan-300/30 bg-cyan-300/10 px-4 py-3 text-sm font-bold text-cyan-100">All parts</Link>
...
export default async function IsomerismPartRoute({ params }: { params: Promise<{ part: string }> }) {
  const { part } = await params; const number = Number(part);
  if (!Number.isInteger(number) || !isomerismParts.some((item) => item.number === number)) notFound();
  return <><IsomerismPartView number={number} /><PartPager number={number} /></>;
}
