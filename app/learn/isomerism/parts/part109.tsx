...
export default function IsomerismPart109() {
  return <main className="min-h-screen bg-[#08111f] px-4 py-8 text-white sm:px-6 sm:py-12"><article className="mx-auto max-w-5xl"><Link href="/learn/isomerism" className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-100">Isomerism</Link><header className="mt-6 rounded-3xl border border-cyan-300/15 bg-[#08111f] p-6 sm:p-9"><p className="text-xs font-black uppercase tracking-[.24em] text-cyan-200">Isomerism source · page 99 · Part 109</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Skew conformers and the rotation-energy profile</h1><p className="mt-3 leading-7 text-[var(--text-body)]">This page relates the named anti, gauche and eclipsed arrangements to a continuous rotation about a C-C sigma bond.</p></header><section className="mt-7 grid gap-5 lg:grid-cols-2"><article className="rounded-2xl border border-white/10 bg-white/[.035] p-5"><h2 className="text-2xl font-black text-cyan-100">Skew conformers</h2><p className="mt-3 text-sm leading-7 text-[var(--text-body)]">The source calls the infinitely many conformers other than the named anti, gauche, partially eclipsed and fully eclipsed forms <b>skew conformers</b>. They are real instantaneous arrangements, but are not the limiting named positions.</p><h3 className="mt-6 text-lg font-bold text-fuchsia-200">Dihedral (torsional) angle</h3><p className="mt-2 text-sm leading-7 text-[var(--text-body)]">θ is the angle between two intersecting planes that contain the valencies of the front and rear atoms when the molecule is viewed along the central bond.</p><DihedralAngle /></article><article className="rounded-2xl border border-white/10 bg-white/[.035] p-5"><h2 className="text-2xl font-black text-cyan-100">Potential-energy diagram</h2><p className="mt-3 text-sm leading-7 text-[var(--text-body)]">As the bond rotates through 0°, 60°, 120°, 180°, 240°, 300° and back to 360°, the energy varies continuously. The source marks the repeated extrema a-f on this profile.</p><EnergyProfile /><p className="mt-3 text-sm leading-7 text-[var(--text-body)]">Minima correspond to the more stable staggered arrangements; maxima correspond to eclipsing interactions. The exact barrier height is substituent-dependent.</p></article></section><section className="mt-7 rounded-2xl border border-amber-300/25 bg-amber-300/[.07] p-5 text-sm leading-7 text-amber-50"><b>Source conclusion:</b> total conformers are infinite because rotation is continuous, while the number of stable limiting conformations is finite and must be counted from the energy minima.</section></article></main>;
}
