      ...
      <div className="space-y-12 text-white">
        ...
        <section className="space-y-6">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="rounded-3xl border border-[var(--border)] bg-[var(--surface)]/80 p-6 space-y-4">
        ...
        {chapter && (
          <section className="rounded-3xl border border-[var(--border)] bg-[var(--surface)]/60 p-6 sm:p-8 space-y-6">
            ...
            <div className="space-y-4">
              {chapter.solvedExamples.map((ex, idx) => (
                <div key={idx} className="rounded-2xl border border-[var(--border)] bg-[var(--background)]/80 p-5 space-y-3">
                  <p className="text-sm font-bold text-cyan-300">Problem {idx + 1}: {ex.q}</p>
                  <div className="text-xs text-[var(--text-body)] bg-[var(--surface)]/60 p-3 rounded-xl border border-[var(--border)] font-mono space-y-1">
                    {ex.steps.map((st, i) => (
                      <p key={i}>• {st}</p>
                    ))}
