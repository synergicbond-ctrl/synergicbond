...
export default function FormulasPage() {
  ...
  return (
    <div className="min-h-screen bg-[var(--background)] p-6 md:p-12 max-w-5xl mx-auto space-y-12">
      <header>
        <h1 className="text-4xl font-black text-[var(--foreground)]">Formula Vault</h1>
      ...
      {chapters.map(([chapterId, formulas]) => (
        <section key={chapterId}>
          <h2 className="text-xl font-black text-[var(--foreground)] mb-4 pb-2 border-b border-[var(--border)]">
            {CHAPTER_LABELS[chapterId] ?? chapterId}
          </h2>
          <div className="space-y-4">
            {formulas.map((f, i) => (
              <div key={i} className="bg-[var(--surface)] rounded-lg p-6 border border-[var(--border)]">
                <h3 className="font-bold text-[var(--foreground)] mb-3">{f.title}</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-[var(--text-muted)]">
                  <div><span className="font-semibold text-[var(--foreground)]">Meaning: </span>{f.meaning}</div>
                  <div><span className="font-semibold text-[var(--foreground)]">Units: </span>{f.units}</div>
                  {f.shortcut && (
                  ...
                  {f.pyqContext && (
                    <div className="md:col-span-2 text-xs text-[var(--text-muted)] italic">PYQ context: {f.pyqContext}</div>
                  )}
                </div>
              </div>
            ))}
