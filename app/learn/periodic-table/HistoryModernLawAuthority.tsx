...
function TelluricScrew() {
  return (
    <figure className="mx-auto my-5 max-w-xl rounded-lg border border-violet-300/20 bg-slate-950/55 p-4">
...
function MoseleyPlot() {
  return (
    <figure className="my-5 rounded-lg border border-rose-300/20 bg-slate-950/55 p-4">
...
function PeriodicTableGraphic() {
  ...
  return (
    <div className="my-6 overflow-x-auto rounded-lg border border-white/10 bg-slate-950/55 p-4">
...
export default function HistoryModernLawAuthority() {
  return (
    <article className="mx-auto max-w-7xl px-4 pb-20 pt-6 text-slate-200 md:px-6">
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{triads.map((triad)=><div key={triad.title} className="rounded-lg border border-white/10 bg-white/[0.035] p-4"><h3 className="font-black text-cyan-200">{triad.title}</h3><div className="mt-3 grid grid-cols-3 gap-2 text-center"><span className="rounded-lg bg-slate-800 p-2">{triad.a}</span><span className="rounded-lg bg-amber-300/15 p-2 font-bold text-amber-100">{triad.b}</span><span className="rounded-lg bg-slate-800 p-2">{triad.c}</span></div><div className="mt-3 text-center text-xs text-emerald-300">{triad.calc}</div></div>)}</div>
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-2"><div className="rounded-lg border border-white/10 bg-slate-950/40 p-4"><h3 className="font-black text-amber-200">Musical analogy</h3><div className="mt-3 grid grid-cols-8 overflow-hidden rounded-lg border border-white/10 text-center text-sm">{["sa","re","ga","ma","pa","dha","ni","sa"].map((x,i)=><div key={i} className={`p-2 ${i===7?"bg-amber-300/15 text-amber-100":"bg-white/[0.03]"}`}>{x}<div className="text-xs text-slate-500">{i+1}</div></div>)}</div></div><div className="rounded-lg border border-white/10 bg-slate-950/40 p-4"><h3 className="font-black text-cyan-200">Representative arrangement</h3><div className="mt-3 grid grid-cols-7 text-center text-sm">{["Li","Be","B","C","N","O","F","Na","Mg","Al","Si","P","S","Cl","K","Ca","—","—","—","—","—"].map((x,i)=><div key={i} className="border border-white/10 p-2">{x}</div>)}</div></div></div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-white/10 bg-white/[0.035] p-5"><h3 className="font-black text-cyan-200">Systematic study</h3><p className="mt-2 leading-7 text-slate-300">Elements were organised into families, making it possible to learn one representative element and infer related behaviour down its group.</p></div><div className="rounded-lg border border-white/10 bg-white/[0.035] p-5"><h3 className="font-black text-cyan-200">Accommodation of later families</h3><p className="mt-2 leading-7 text-slate-300">The noble gases could be inserted as a zero group without destroying the main architecture.</p></div></div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Hydrogen</h3><p className="mt-2 text-sm leading-6 text-slate-300">Hydrogen resembles group 1 through its 1s¹ configuration and H⁺ formation, but also resembles group 17 through its need for one electron and H⁻ formation.</p></div><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Isotopes</h3><p className="mt-2 text-sm leading-6 text-slate-300">Different atomic weights would imply separate positions, even though isotopes are chemically the same element.</p></div><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Similar elements separated</h3><p className="mt-2 text-sm leading-6 text-slate-300">Elements such as Pt and Au could be split into different groups despite substantial chemical resemblance.</p></div><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Dissimilar elements combined</h3><p className="mt-2 text-sm leading-6 text-slate-300">Alkali metals and coinage metals were both placed in group I, although their chemistry differs greatly.</p></div></div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">{blockCards.map((card)=><div key={card.block} className={`rounded-lg border-l-4 ${card.accent} border-y border-r border-white/10 bg-white/[0.035] p-5`}><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-black text-white">{card.block}</h3><span className="rounded-lg bg-slate-950/70 px-2 py-1 text-xs font-bold text-cyan-200">{card.config}</span></div><div className="mt-2 text-sm font-bold text-amber-200">{card.groups}</div><p className="mt-2 text-sm leading-6 text-slate-300">{card.detail}</p></div>)}</div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-sky-300/20 bg-sky-400/5 p-5"><h3 className="font-black text-sky-200">Named d-block families</h3><p className="mt-3 text-sm leading-7 text-slate-300">Coinage metals: Cu, Ag, Au. Platinum metals: Ru, Rh, Pd, Os, Ir, Pt. Mercury is the only metal liquid at ordinary room temperature. Technetium was the first element produced artificially.</p></div><div className="rounded-lg border border-violet-300/20 bg-violet-400/5 p-5"><h3 className="font-black text-violet-200">Inner-transition series</h3><p className="mt-3 text-sm leading-7 text-slate-300">Lanthanoids: Ce–Lu, filling 4f. Actinoids: Th–Lr, filling 5f; all are radioactive. Lanthanoid contraction is the steady size decrease across the 4f series.</p></div></div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3"><div className="rounded-lg border border-sky-300/20 bg-sky-400/5 p-5"><h3 className="text-lg font-black text-sky-200">Metals</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-300"><li>Mostly solid; mercury is liquid.</li><li>Good thermal and electrical conductors.</li><li>Usually malleable and ductile.</li><li>Tend to lose electrons and form basic oxides.</li><li>Ga and Cs melt near ordinary body-temperature range.</li></ul></div><div className="rounded-lg border border-emerald-300/20 bg-emerald-400/5 p-5"><h3 className="text-lg font-black text-emerald-200">Non-metals</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-slate-300"><li>Solids or gases; bromine is liquid.</li><li>Solid non-metals are generally brittle.</li><li>Often lower melting and boiling points.</li><li>Tend to gain electrons and form acidic oxides.</li><li>Boron and carbon are high-melting special cases.</li></ul></div><div className="rounded-lg border border-amber-300/20 bg-amber-400/5 p-5"><h3 className="text-lg font-black text-amber-200">Metalloids</h3><p className="mt-3 text-sm leading-6 text-slate-300">Borderline elements with mixed metallic and non-metallic behaviour, often semiconducting:</p><div className="mt-4 flex flex-wrap gap-2">{["B","Si","Ge","As","Sb","Te"].map((x)=><span key={x} className="rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1 text-sm font-bold text-amber-100">{x}</span>)}</div></div></div>
