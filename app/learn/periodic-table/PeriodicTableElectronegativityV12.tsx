...
function TableX({ columns, rows, minWidth = 720, accent = T.cyan }: { columns: string[]; rows: React.ReactNode[][]; minWidth?: number; accent?: string }) {
  return <div style={{ width: "100%", overflowX: "auto", border: `1px solid ${T.border}`, borderRadius: 13, margin: "11px 0" }}><table style={{ width: "100%", minWidth, borderCollapse: "collapse", fontFamily: T.sans, fontSize: 13.1 }}><thead><tr>{columns.map((c, ci) => <th key={`${c}-${ci}`} style={{ color: accent, padding: "9px 11px", textAlign: "left", borderBottom: `2px solid ${accent}`, textTransform: "uppercase", fontSize: 11.2, letterSpacing: .5, whiteSpace: "nowrap" }}>{c}</th>)}</tr></thead><tbody>{rows.map((r, ri) => <tr key={ri} style={{ background: ri % 2 ? "rgba(255,255,255,.018)" : "transparent" }}>{r.map((cell, ci) => <td key={ci} style={{ color: ci === 0 ? T.text : T.dim, fontWeight: ci === 0 ? 750 : 450, padding: "8px 11px", borderBottom: `1px solid ${T.border}`, verticalAlign: "top", lineHeight: 1.48 }}>{cell}</td>)}</tr>)}</tbody></table></div>;
}
