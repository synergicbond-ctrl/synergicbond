          ...
          {EXAM_DIRECTORY.map((e) => {
            ...
            return (
              <Link
                key={e.id}
                href={e.href}
                className="group rounded-2xl border bg-[var(--surface)] p-6 transition hover:-translate-y-1"
...
function KeyValue({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2.5 rounded-lg border border-slate-900 bg-[var(--background)]/40 px-3 py-2">
