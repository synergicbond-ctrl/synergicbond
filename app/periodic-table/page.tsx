    ...
    <main className="min-h-screen bg-[var(--background)] text-white">
            ...
            {TRENDS.map((v) => (
              <button
                ...
                className={`text-[11px] font-semibold px-3 py-1.5 rounded-full transition ${
                  trendProp === v.id ? "bg-cyan-500 text-black" : "bg-[var(--surface)] border border-white/[0.06] text-[var(--text-muted)] hover:text-white"
                }`}
              >
                {v.label}
              </button>
      ...
      {active && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-black/70 backdrop-blur-sm" onClick={() => setActive(null)}>
          <div className="w-full max-w-md rounded-3xl border border-white/[0.08] bg-[var(--surface)] shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
