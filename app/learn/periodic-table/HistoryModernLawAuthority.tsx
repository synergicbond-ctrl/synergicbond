...
function SectionTitle({ code, children }: { code: string; children: ReactNode }) {
  return (
    <div className="mt-10 flex items-center gap-3 rounded-lg border border-cyan-400/25 bg-gradient-to-r from-slate-950 via-blue-950 to-slate-900 px-4 py-3 shadow-[0_16px_45px_rgba(8,47,73,0.28)]">
      <span className="inline-flex min-w-10 items-center justify-center rounded-lg bg-amber-300 px-2 py-1 text-xs font-black text-[var(--foreground)]">{code}</span>
...
function Note({ children, tone = "blue" }: { children: ReactNode; tone?: "blue" | "amber" | "red" | "green" }) {
  ...
  return <div className={`my-4 rounded-lg border px-4 py-3 text-sm leading-6 ${styles}`}>{children}</div>;
}

function DataTable<T extends Record<string, ReactNode>>({ rows, columns }: { rows: T[]; columns: TableColumn<T>[] }) {
  return (
    <div className="my-4 overflow-x-auto rounded-lg border border-white/10 bg-[var(--surface)]/40">
      <table className="min-w-full border-collapse text-sm">
        ...
        <tbody>{rows.map((row, rowIndex) => <tr key={rowIndex} className="odd:bg-white/[0.025] even:bg-blue-500/[0.03]">{columns.map((column) => <td key={String(column.key)} className={`border-b border-white/5 px-3 py-2.5 align-top text-[var(--text-faint)] ${column.align === "center" ? "text-center" : column.align === "right" ? "text-right" : "text-left"}`}>{row[column.key]}</td>)}</tr>)}</tbody>
      </table>
    </div>
  );
}
...
function TelluricScrew() {
  return (
    <figure className="mx-auto my-5 max-w-xl rounded-lg border border-violet-300/20 bg-[var(--surface)]/55 p-4">
...
function MoseleyPlot() {
  return (
    <figure className="my-5 rounded-lg border border-rose-300/20 bg-[var(--surface)]/55 p-4">
...
function PeriodicTableGraphic() {
  ...
  return (
    <div className="my-6 overflow-x-auto rounded-lg border border-white/10 bg-[var(--surface)]/55 p-4">
      <div className="min-w-[1000px]">
        <div className="grid grid-cols-[42px_repeat(18,minmax(42px,1fr))] gap-1 text-center text-[10px] text-[var(--text-muted)]">
          <div />{Array.from({length:18},(_,i)=><div key={i} className="py-1 font-bold">{i+1}</div>)}
          {Array.from({length:7},(_,p)=>{
            const period=p+1;
            return <React.Fragment key={period}><div className="flex items-center justify-center font-bold text-cyan-200">{period}</div>{Array.from({length:18},(_,g)=>{const element=elements.find((item)=>item.period===period&&item.group===g+1);return <div key={g} className={`min-h-14 rounded-md border p-1 ${element ? blockClass[element.block] : "border-transparent"}`}>{element && <><div className="text-[9px] text-[var(--text-muted)]">{element.z}</div><div className="text-sm font-black text-white">{element.symbol}</div></>}</div>})}</React.Fragment>;
          })}
        </div>
      ...
      <div className="mt-4 flex flex-wrap gap-3 text-xs text-[var(--text-faint)]"><span>🌹 s-block</span><span>💠 d-block</span><span>🌿 p-block</span><span>🔮 f-block</span></div>
    </div>
  );
}
...
export default function HistoryModernLawAuthority() {
  return (
    <article className="mx-auto max-w-7xl px-4 pb-20 pt-6 text-[var(--text-faint)] md:px-6">
      <header className="relative overflow-hidden rounded-lg border border-cyan-400/20 bg-[radial-gradient(circle_at_top_right,rgba(37,99,235,0.3),transparent_38%),linear-gradient(135deg,#020617,#0f172a_48%,#082f49)] px-5 py-10 shadow-[0_30px_90px_rgba(2,132,199,0.16)] md:px-9 md:py-14">
        ...
        <div className="relative">
          ...
          <p className="mt-5 max-w-4xl text-base leading-7 text-[var(--text-faint)] md:text-lg">A rigorous journey from the earliest attempts at classification to atomic number, electronic configuration, the long-form table, block structure, group logic and modern nomenclature.</p>
          <div className="mt-7 grid gap-3 sm:grid-cols-3"><div className="rounded-lg border border-white/10 bg-white/5 p-3"><div className="text-2xl font-black text-cyan-300">7</div><div className="text-xs text-[var(--text-muted)]">periods</div></div><div className="rounded-lg border border-white/10 bg-white/5 p-3"><div className="text-2xl font-black text-amber-300">18</div><div className="text-xs text-[var(--text-muted)]">groups</div></div><div className="rounded-lg border border-white/10 bg-white/5 p-3"><div className="text-2xl font-black text-emerald-300">118</div><div className="text-xs text-[var(--text-muted)]">recognised elements</div></div></div>
        </div>
      </header>

      <SectionTitle code="1">Why classification became necessary</SectionTitle>
      <p className="mt-4 leading-7 text-[var(--text-faint)]">A periodic table is a tabular arrangement in which elements are ordered by a fundamental property while chemically similar elements are brought into the same vertical family. As the number of known elements increased, memorising each one independently became impractical. Chemists therefore searched for an ordering principle that would make physical and chemical behaviour recur in a recognisable pattern.</p>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-sky-300/20 bg-sky-400/5 p-5"><h3 className="text-lg font-black text-sky-200">Lavoisier’s classification, 1789</h3><p className="mt-2 leading-7 text-[var(--text-faint)]">Known substances were separated into metals—lustrous, conducting and commonly forming basic oxides—and non-metals—generally dull, poorly conducting and commonly forming acidic oxides.</p><Note tone="red"><b>Limitation:</b> the division was too coarse and offered no proper position for intermediate metalloids such as silicon and arsenic.</Note></div><div className="rounded-lg border border-amber-300/20 bg-amber-400/5 p-5"><h3 className="text-lg font-black text-amber-200">Prout’s hypothesis</h3><p className="mt-2 leading-7 text-[var(--text-faint)]">Prout proposed that hydrogen was the fundamental building unit and that every atomic weight should be an exact whole-number multiple of hydrogen’s atomic weight.</p><Note tone="red">More accurate measurements, including chlorine near 35.5, showed that the proposal could not be retained in its original form.</Note></div></div>

      <SectionTitle code="B">Döbereiner’s law of triads</SectionTitle>
      <p className="mt-4 leading-7 text-[var(--text-faint)]">When three similar elements were placed in increasing atomic-weight order, the atomic weight of the middle element was approximately the arithmetic mean of the first and third.</p>
      <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">{triads.map((triad)=><div key={triad.title} className="rounded-lg border border-white/10 bg-white/[0.035] p-4"><h3 className="font-black text-cyan-200">{triad.title}</h3><div className="mt-3 grid grid-cols-3 gap-2 text-center"><span className="rounded-lg bg-[var(--surface)] p-2">{triad.a}</span><span className="rounded-lg bg-amber-300/15 p-2 font-bold text-amber-100">{triad.b}</span><span className="rounded-lg bg-[var(--surface)] p-2">{triad.c}</span></div><div className="mt-3 text-center text-xs text-emerald-300">{triad.calc}</div></div>)}</div>
      ...
      <p className="mt-4 leading-7 text-[var(--text-faint)]">Elements were wound in increasing atomic-weight order around a cylinder divided into sixteen vertical units. Elements separated by one full turn appeared one above another and often showed related chemistry. This was the first explicit use of periodicity in a geometric classification.</p>
      <TelluricScrew />
      ...
      <p className="mt-4 leading-7 text-[var(--text-faint)]">Newlands arranged the 56 known elements in increasing atomic weight and observed that the eighth element often resembled the first, analogous to the recurrence of the first note at the eighth position in a musical scale.</p>
      <div className="mt-4 grid gap-5 lg:grid-cols-2"><div className="rounded-lg border border-white/10 bg-[var(--surface)]/40 p-4"><h3 className="font-black text-amber-200">Musical analogy</h3><div className="mt-3 grid grid-cols-8 overflow-hidden rounded-lg border border-white/10 text-center text-sm">{["sa","re","ga","ma","pa","dha","ni","sa"].map((x,i)=><div key={i} className={`p-2 ${i===7?"bg-amber-300/15 text-amber-100":"bg-white/[0.03]"}`}>{x}<div className="text-xs text-[var(--text-muted)]">{i+1}</div></div>)}</div></div><div className="rounded-lg border border-white/10 bg-[var(--surface)]/40 p-4"><h3 className="font-black text-cyan-200">Representative arrangement</h3><div className="mt-3 grid grid-cols-7 text-center text-sm">{["Li","Be","B","C","N","O","F","Na","Mg","Al","Si","P","S","Cl","K","Ca","—","—","—","—","—"].map((x,i)=><div key={i} className="border border-white/10 p-2">{x}</div>)}</div></div></div>
      ...
      <div className="mt-4 rounded-lg border border-cyan-300/20 bg-cyan-400/5 p-5 text-center text-lg font-black text-cyan-100">Atomic volume = gram atomic mass / density &nbsp; (cm³ mol⁻¹)</div>
      <p className="mt-4 leading-7 text-[var(--text-faint)]">Meyer plotted atomic volume against atomic weight and obtained a recurring wave. Alkali metals appeared near sharp peaks, alkaline-earth metals on descending sections, transition elements and several metalloids near broad valleys, and halogens on rising sections toward the next alkali-metal maximum.</p>
      ...
      <p className="leading-7 text-[var(--text-faint)]">Mendeleev’s decisive advance was predictive flexibility. He retained chemically similar elements in the same column, deliberately left gaps for unknown elements and occasionally reversed strict atomic-weight order when chemical evidence demanded it.</p>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-white/10 bg-white/[0.035] p-5"><h3 className="font-black text-cyan-200">Systematic study</h3><p className="mt-2 leading-7 text-[var(--text-faint)]">Elements were organised into families, making it possible to learn one representative element and infer related behaviour down its group.</p></div><div className="rounded-lg border border-white/10 bg-white/[0.035] p-5"><h3 className="font-black text-cyan-200">Accommodation of later families</h3><p className="mt-2 leading-7 text-[var(--text-faint)]">The noble gases could be inserted as a zero group without destroying the main architecture.</p></div></div>
      ...
      <p className="leading-7 text-[var(--text-faint)]">For beryllium, equivalent weight ≈ 4.5. Assuming valency 3 gives 13.5, which did not fit a chemically sensible position. Valency 2 gives 9, correctly placing Be between Li and B. Similar table-guided corrections were applied to accepted values for Be, In, Au, Pt and U.</p>

      <SectionTitle code="F2">Demerits of Mendeleev’s table</SectionTitle>
      <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Hydrogen</h3><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">Hydrogen resembles group 1 through its 1s¹ configuration and H⁺ formation, but also resembles group 17 through its need for one electron and H⁻ formation.</p></div><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Isotopes</h3><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">Different atomic weights would imply separate positions, even though isotopes are chemically the same element.</p></div><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Similar elements separated</h3><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">Elements such as Pt and Au could be split into different groups despite substantial chemical resemblance.</p></div><div className="rounded-lg border border-rose-300/20 bg-rose-500/5 p-4"><h3 className="font-black text-rose-200">Dissimilar elements combined</h3><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">Alkali metals and coinage metals were both placed in group I, although their chemistry differs greatly.</p></div></div>
      <h3 className="mt-6 text-lg font-black text-amber-200">Anomalous atomic-weight pairs</h3>
      <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{["Ar (39.9) before K (39.1)","Co (58.9) before Ni (58.7)","Te (127.6) before I (126.9)","Th (232) before Pa (231)"].map((pair)=><div key={pair} className="rounded-lg border border-amber-300/20 bg-amber-400/5 p-3 text-center font-bold text-amber-100">{pair}</div>)}</div>
      ...
      <p className="leading-7 text-[var(--text-faint)]">Atomic number resolves the anomalous pairs automatically: Ar has Z = 18 and therefore precedes K with Z = 19 even though Ar has slightly greater atomic mass. The ordering also aligns directly with electron configuration, which controls chemistry.</p>

      <SectionTitle code="H">Cause of periodicity</SectionTitle>
      <p className="mt-4 leading-7 text-[var(--text-faint)]">As Z increases one unit at a time, electrons enter orbitals in a regular sequence. A new period begins when filling starts in a new principal shell. Elements recur in the same group when they acquire analogous valence-shell configurations. Thus Li 2s¹, Na 3s¹, K 4s¹, Rb 5s¹ and Cs 6s¹ display related chemistry because each ends in ns¹.</p>
      ...
      <p className="mt-4 leading-7 text-[var(--text-faint)]">The long form arranges elements by increasing Z into seven periods and eighteen IUPAC groups. The f-block is displayed separately only to keep the main table compact; electronically it belongs within periods 6 and 7.</p>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2">{blockCards.map((card)=><div key={card.block} className={`rounded-lg border-l-4 ${card.accent} border-y border-r border-white/10 bg-white/[0.035] p-5`}><div className="flex items-start justify-between gap-3"><h3 className="text-lg font-black text-white">{card.block}</h3><span className="rounded-lg bg-[var(--surface)]/70 px-2 py-1 text-xs font-bold text-cyan-200">{card.config}</span></div><div className="mt-2 text-sm font-bold text-amber-200">{card.groups}</div><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">{card.detail}</p></div>)}</div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-3"><div className="rounded-lg border border-white/10 bg-white/[0.035] p-4"><h3 className="font-black text-cyan-200">Period</h3><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">Highest principal quantum number present.</p></div><div className="rounded-lg border border-white/10 bg-white/[0.035] p-4"><h3 className="font-black text-amber-200">Block</h3><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">Subshell receiving the differentiating electron.</p></div><div className="rounded-lg border border-white/10 bg-white/[0.035] p-4"><h3 className="font-black text-emerald-200">Group</h3><p className="mt-2 text-sm leading-6 text-[var(--text-faint)]">s: ns count; p: valence count + 10; d: ns + (n−1)d; f: group 3 convention.</p></div></div>
      ...
      <div className="mt-4 grid gap-4 md:grid-cols-2"><div className="rounded-lg border border-sky-300/20 bg-sky-400/5 p-5"><h3 className="font-black text-sky-200">Named d-block families</h3><p className="mt-3 text-sm leading-7 text-[var(--text-faint)]">Coinage metals: Cu, Ag, Au. Platinum metals: Ru, Rh, Pd, Os, Ir, Pt. Mercury is the only metal liquid at ordinary room temperature. Technetium was the first element produced artificially.</p></div><div className="rounded-lg border border-violet-300/20 bg-violet-400/5 p-5"><h3 className="font-black text-violet-200">Inner-transition series</h3><p className="mt-3 text-sm leading-7 text-[var(--text-faint)]">Lanthanoids: Ce–Lu, filling 4f. Actinoids: Th–Lr, filling 5f; all are radioactive. Lanthanoid contraction is the steady size decrease across the 4f series.</p></div></div>
      ...
      <div className="mt-5 grid gap-4 lg:grid-cols-3"><div className="rounded-lg border border-sky-300/20 bg-sky-400/5 p-5"><h3 className="text-lg font-black text-sky-200">Metals</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-[var(--text-faint)]"><li>Mostly solid; mercury is liquid.</li><li>Good thermal and electrical conductors.</li><li>Usually malleable and ductile.</li><li>Tend to lose electrons and form basic oxides.</li><li>Ga and Cs melt near ordinary body-temperature range.</li></ul></div><div className="rounded-lg border border-emerald-300/20 bg-emerald-400/5 p-5"><h3 className="text-lg font-black text-emerald-200">Non-metals</h3><ul className="mt-3 space-y-2 text-sm leading-6 text-[var(--text-faint)]"><li>Solids or gases; bromine is liquid.</li><li>Solid non-metals are generally brittle.</li><li>Often lower melting and boiling points.</li><li>Tend to gain electrons and form acidic oxides.</li><li>Boron and carbon are high-melting special cases.</li></ul></div><div className="rounded-lg border border-amber-300/20 bg-amber-400/5 p-5"><h3 className="text-lg font-black text-amber-200">Metalloids</h3><p className="mt-3 text-sm leading-6 text-[var(--text-faint)]">Borderline elements with mixed metallic and non-metallic behaviour, often semiconducting:</p><div className="mt-4 flex flex-wrap gap-2">{["B","Si","Ge","As","Sb","Te"].map((x)=><span key={x} className="rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1 text-sm font-bold text-amber-100">{x}</span>)}</div></div></div>

      <SectionTitle code="Q">Temporary nomenclature for elements above Z = 100</SectionTitle>
      <p className="mt-4 leading-7 text-[var(--text-faint)]">Before an official name is approved, a systematic name is assembled digit by digit from roots: 0 nil, 1 un, 2 bi, 3 tri, 4 quad, 5 pent, 6 hex, 7 sept, 8 oct and 9 enn, followed by “ium.” For Z = 105: un + nil + pent + ium → unnilpentium, symbol Unp.</p>
      ...
      <div className="mt-7 rounded-lg border border-emerald-300/20 bg-emerald-400/5 p-5 text-center font-bold leading-7 text-emerald-50">Lavoisier → Prout → Döbereiner → Chancourtois → Newlands → Lothar Meyer → Mendeleev → Moseley → modern atomic-number and electronic-configuration table.</div>
    </article>
  );
}
