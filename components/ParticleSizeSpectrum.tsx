...
export default function ParticleSizeSpectrum() {
  ...
  return (
    <div className="p-6 rounded-2xl bg-white dark:bg-[var(--surface)] border border-[var(--border)] dark:border-[var(--border-strong)] shadow-sm space-y-6">
      ...
      {/* Scale ruler */}
      <div className="bg-slate-100 dark:bg-[var(--surface-2)] p-4 rounded-xl">
        <div className="flex items-end gap-1 h-16 mb-2 justify-between px-2">
          {[1, 10, 100, 1000, 10000].map((val, idx) => (
            <div key={idx} className="flex flex-col items-center flex-1">
              ...
              <span className="text-xs text-slate-600 dark:text-[var(--text-muted)] mt-1">
                {val < 1000 ? val : "1μ"}
              </span>
            </div>
          ))}
        </div>
        <p className="text-xs text-slate-500 dark:text-[var(--text-muted)] text-center font-mono">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {ranges.map((range, idx) => (
          ...
          >
            ...
            <p className="text-xs text-slate-600 dark:text-[var(--text-muted)] mb-3">
            ...
            <div className="space-y-2">
              <div>
                <p className="text-xs font-semibold text-slate-600 dark:text-[var(--text-muted)] mb-1">
                  Examples:
                </p>
                <div className="space-y-1">
                  {range.examples.map((ex, eidx) => (
                    <p key={eidx} className="text-xs text-slate-700 dark:text-[var(--text-body)]">
                      • {ex}
                    </p>
                  ))}
                </div>
              ...
              <div className={`mt-3 p-2 rounded ${range.tyndall ? "bg-white dark:bg-[var(--surface-2)]" : "bg-slate-200 dark:bg-slate-700"}`}>
                <p className="text-xs font-semibold">
                  {range.tyndall ? "✓ Tyndall visible" : "✗ NO Tyndall effect"}
                </p>
                <p className="text-xs text-slate-600 dark:text-[var(--text-muted)]">
