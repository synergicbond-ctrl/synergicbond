...
const markdownComponents: Components = {
  h1: ({ children }) => {
    ...
    return (
      <section className="mt-12 first:mt-0">
        <h2 id={slugify(text)} className="scroll-mt-24 border-t border-[var(--border)] pt-10 font-display text-3xl font-black leading-tight tracking-tight text-transparent bg-clip-text bg-[linear-gradient(90deg,#fbbf24,#fb7185,#22d3ee)] sm:text-4xl">
          {children}
        </h2>
      </section>
    );
  ...
  h2: ({ children }) => {
    const text = flattenText(children);
    return <h3 id={slugify(text)} className="scroll-mt-24 pt-7 font-display text-2xl font-black leading-tight text-transparent bg-clip-text bg-[linear-gradient(90deg,#22d3ee,#a78bfa)] sm:text-[28px]">{children}</h3>;
  },
  h3: ({ children }) => <h4 className="pt-4 font-display text-xl font-black leading-tight text-transparent bg-clip-text bg-[linear-gradient(90deg,#fb7185,#fbbf24)] sm:text-[22px]">{children}</h4>,
  h4: ({ children }) => <h5 className="pt-2 font-display text-base font-black text-violet-200 sm:text-lg">{children}</h5>,
  ...
  strong: ({ children }) => <strong className="font-black text-transparent bg-clip-text bg-[linear-gradient(90deg,#fbbf24,#22d3ee)]">{children}</strong>,
  ...
  a: ({ href, children }) => <a href={href} className="font-bold text-cyan-300 underline decoration-cyan-300/30 underline-offset-4 hover:text-violet-200">{children}</a>,
