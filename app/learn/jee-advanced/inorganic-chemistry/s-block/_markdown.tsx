...
const markdownComponents: Components = {
  h1: ({ children }) => {
    ...
    return (
      <section className="mt-12 first:mt-0">
        <h2 id={slugify(text)} className="scroll-mt-24 border-t border-[var(--border)] pt-10 font-display text-3xl font-black leading-tight tracking-tight text-amber-300 sm:text-4xl">
          {children}
        </h2>
      </section>
    );
  ...
  h2: ({ children }) => {
    const text = flattenText(children);
    return <h3 id={slugify(text)} className="scroll-mt-24 pt-7 font-display text-2xl font-black leading-tight text-cyan-300 sm:text-[28px]">{children}</h3>;
  },
  h3: ({ children }) => <h4 className="pt-4 font-display text-xl font-black leading-tight text-fuchsia-300 sm:text-[22px]">{children}</h4>,
  h4: ({ children }) => <h5 className="pt-2 font-display text-base font-black text-violet-300 sm:text-lg">{children}</h5>,
