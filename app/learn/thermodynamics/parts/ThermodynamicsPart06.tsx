    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {modes.map(([title, meaning, note], index) => (
        <article key={title} className="rounded-2xl border border-white/10 bg-[var(--surface)]/70 p-5">
          ...
          <h3 className="mt-2 text-lg font-black text-white">{title}</h3>
          <p className="mt-2 text-sm leading-6 text-[var(--foreground)]">{meaning}</p>
    ...
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
      {factors.map((factor) => (
        <article key={factor.title} className={`rounded-3xl border p-5 ${toneMap[factor.tone]}`}>
          ...
          <p className="mt-3 text-sm leading-6 text-[var(--foreground)]">{factor.text}</p>
        </article>
      ))}
    </div>
  );
    ...
    <main className="min-h-screen bg-[#020617] text-slate-100">
            ...
            <div className="rounded-[2rem] border border-white/10 bg-white/[0.05] p-6 shadow-2xl shadow-cyan-950/25">
              ...
              <div className="mt-4 space-y-3">
                <div className="rounded-xl bg-emerald-400/10 p-4">
                  ...
                  <p className="mt-1 text-sm text-[var(--text-body)]">translation, rotation, vibration, interaction and electronic energy</p>
                </div>
                <div className="rounded-xl bg-amber-400/10 p-4">
                  <p className="font-black text-amber-200">Outside U</p>
                  <p className="mt-1 text-sm text-[var(--text-body)]">motion and external potential energy of the system as a whole</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="mx-auto max-w-7xl space-y-7 px-5 py-9 md:px-8 md:py-12">
        ...
        >
          ...
          <p className="rounded-2xl border border-rose-300/20 bg-rose-300/[0.06] p-4 text-sm leading-6 text-[var(--foreground)]">
