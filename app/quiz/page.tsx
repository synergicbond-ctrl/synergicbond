...
export default function QuizPage() {
  ...
  if (!started) {
    return (
      <div className="min-h-screen bg-[var(--background)] text-white flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-lg space-y-6">
          <h1 className="text-3xl font-black text-center">Quiz Mode</h1>
          <p className="text-[var(--text-muted)] text-center text-sm">5 AI-generated questions per chapter</p>

          <div className="bg-[var(--surface)] rounded-lg p-6 space-y-4">
            <label className="text-xs font-bold text-[var(--text-muted)] uppercase tracking-wider block">Select Chapter</label>
            <div className="grid grid-cols-1 gap-2 max-h-72 overflow-y-auto pr-1">
              {CHAPTERS.map((ch) => (
                <button key={ch.id} onClick={() => setChapterId(ch.id)}
                  className={`w-full text-left px-4 py-3 rounded-lg text-sm font-semibold border transition ${
                    chapterId === ch.id
                      ? "bg-[var(--accent)] border-[var(--accent)] text-[var(--background)]"
                      : "bg-[var(--surface-2)] border-[var(--border)] text-[var(--text-muted)] hover:border-white/30"
                  }`}>
                  {ch.label}
                </button>
              ))}
            ...
            <button onClick={startQuiz} disabled={loading}
              className="w-full py-4 bg-[var(--accent)] text-[var(--background)] font-black rounded-lg transition hover:opacity-90 disabled:opacity-50">
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[var(--background)] text-white">
        <div className="text-center space-y-6 max-w-sm">
          ...
          <p className="text-5xl font-black text-[var(--accent)]">{score}/{questions.length}</p>
          <p className="text-[var(--text-muted)]">{pct}% accuracy</p>
          <div className="flex gap-3 justify-center">
            <button onClick={() => { setStarted(false); }}
              className="px-6 py-3 bg-[var(--surface-2)] border border-[var(--border)] rounded-lg font-semibold text-sm hover:bg-[var(--surface)] transition">
              Change Chapter
            </button>
            <button onClick={startQuiz}
              className="px-6 py-3 bg-[var(--accent)] text-[var(--background)] rounded-lg font-semibold text-sm hover:opacity-90 transition">
              Retry
            </button>
          </div>
        </div>
    ...
    <div className="min-h-screen bg-[var(--background)] text-white p-6 flex flex-col items-center">
      <div className="w-full max-w-2xl space-y-6">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[var(--text-muted)] text-sm">{current + 1} / {questions.length}</span>
        </div>
        ...
        <div className="bg-[var(--surface)] p-6 rounded-lg border border-[var(--border)] space-y-5">
          ...
          <div className="space-y-3">
            {Object.entries(currentQ.options).map(([key, value]) => (
              <button key={key} onClick={() => handleAnswer(key)} disabled={!!selected}
                className={`w-full p-4 rounded-lg border text-left text-sm font-medium transition ${
                  selected === key
                    ? key === currentQ.answer
                    ...
                    ? "bg-green-600 border-green-500 text-white"
                    : "bg-[var(--surface-2)] border-[var(--border)] hover:border-white/30 disabled:cursor-default"
                }`}>
        ...
        <div className="text-center text-[var(--text-muted)] text-sm">Score: {score}</div>
      </div>
    </div>
  );
}
