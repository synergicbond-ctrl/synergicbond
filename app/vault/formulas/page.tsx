...
export default function FormulasPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-[var(--background)] p-6 md:p-12 max-w-5xl mx-auto space-y-12">
      <header>
      ...
      {chapters.map(([chapterId, formulas]) => (
        <section key={chapterId}>
          <h2 className="text-xl font-black text-slate-800 dark:text-slate-100 mb-4 pb-2 border-b border-[var(--border)] dark:border-[var(--border)]">
          ...
          <div className="space-y-4">
            {formulas.map((f, i) => (
              <div key={i} className="bg-white dark:bg-[var(--surface)] rounded-2xl p-6 border border-[var(--border)] dark:border-[var(--border)]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm text-slate-600 dark:text-[var(--text-muted)]">
                  <div><span className="font-semibold text-slate-700 dark:text-[var(--text-body)]">Meaning: </span>{f.meaning}</div>
                  <div><span className="font-semibold text-slate-700 dark:text-[var(--text-body)]">Units: </span>{f.units}</div>
                  {f.shortcut && (
