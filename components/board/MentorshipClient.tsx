  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <label className="text-sm text-white/70">
            ...
            <input type="date" value={examDate} onChange={(e) => saveDate(e.target.value)}
              className="rounded-lg border border-white/15 bg-[var(--surface)] px-3 py-1.5 text-white/90" />
          </label>
        </div>
      </section>
      ...
      <section>
        ...
        {!plan ? (
          <p className="rounded-lg border border-white/10 bg-white/[0.02] p-4 text-sm text-white/55">Set your exam date above to generate a week-by-week plan.</p>
        ) : (
          <div className="space-y-2">
            {plan.out.map((w) => {
              const active = w.week === plan.currentWeek;
              return (
                <div key={w.week} className={`rounded-lg border p-3 ${active ? "border-[var(--accent)]/40 bg-[var(--accent)]/[0.06]" : "border-white/[0.08] bg-white/[0.02]"}`}>
                  <div className="flex items-center gap-2">
                    <span className={`text-xs font-black ${active ? "text-[var(--accent)]" : "text-white/40"}`}>WEEK {w.week}{active ? " · NOW" : ""}</span>
      ...
      <section>
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          <Link href="/memory?deck=daily" className="flex items-center justify-between rounded-lg border border-white/[0.08] bg-white/[0.02] p-3 hover:border-[var(--accent)]/40"><span className="text-sm font-semibold text-white/90">Daily spaced-recall queue</span><span className="text-[var(--accent)]">→</span></Link>
          <Link href="/revision" className="flex items-center justify-between rounded-lg border border-white/[0.08] bg-white/[0.02] p-3 hover:border-[var(--accent)]/40"><span className="text-sm font-semibold text-white/90">Smart Revision Hub</span><span className="text-[var(--accent)]">→</span></Link>
          <Link href={`${base}/practice`} className="flex items-center justify-between rounded-lg border border-white/[0.08] bg-white/[0.02] p-3 hover:border-[var(--accent)]/40"><span className="text-sm font-semibold text-white/90">Practise weak chapters</span><span className="text-[var(--accent)]">→</span></Link>
          <Link href="/mistakes" className="flex items-center justify-between rounded-lg border border-white/[0.08] bg-white/[0.02] p-3 hover:border-[var(--accent)]/40"><span className="text-sm font-semibold text-white/90">Mistake Journal</span><span className="text-[var(--accent)]">→</span></Link>
        </div>
      </section>
      ...
      <section className="rounded-lg border border-white/10 bg-white/[0.03] p-5">
        ...
        <Link href={`${base}/analytics`} className="mt-3 inline-block rounded-lg bg-[var(--accent)] px-4 py-2 text-sm font-black text-black">Open full report →</Link>
      </section>
      ...
      <section>
        <div className="mb-1 flex items-center justify-between">
          ...
          <Link href="/support" className="text-sm font-semibold text-[var(--accent)] hover:opacity-80">Ask a mentor →</Link>
        </div>
        ...
        <div className="flex gap-2">
          <input value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addNote(); }}
            ...
            className="flex-1 rounded-lg border border-white/10 bg-[var(--surface)] px-3 py-2 text-sm text-white/90 outline-none focus:border-[var(--accent)]/40" />
          <button onClick={addNote} className="rounded-lg border border-[var(--accent)]/40 bg-[var(--accent)]/10 px-4 py-2 text-sm font-bold text-[var(--accent)]">Add</button>
        </div>
        <div className="mt-3 space-y-2">
          {notes.length === 0 ? (
            <p className="rounded-lg border border-white/10 bg-white/[0.02] p-3 text-sm text-white/45">No notes yet.</p>
          ) : notes.map((n) => (
            <div key={n.id} className="flex items-start justify-between gap-3 rounded-lg border border-white/[0.08] bg-white/[0.02] p-3">
