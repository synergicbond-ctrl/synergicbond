...
function DerivRow({ step, result }: { step: string; result: string }) {
  return (
    <div className="grid gap-2 border-b border-white/5 pb-3 pt-1 md:grid-cols-[1fr_2fr]">
      <span className="text-xs font-black uppercase tracking-wider text-[var(--text-muted)]">{step}</span>
...
function FundamentalRelationsSVG() {
  return (
    <figure className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#07111f] p-3 md:p-5">
      ...
      <figcaption className="px-3 pt-3 text-center text-sm leading-6 text-[var(--text-body)]">
...
export default function ThermodynamicsPart25() {
  return (
    <main className="min-h-screen bg-[#020617] text-slate-100">
      <div className="mx-auto max-w-5xl space-y-6 px-4 py-10 md:px-8">
        ...
        <Section index="1" title="The Four Fundamental Differential Relations"
          ...
          <div className="space-y-1 rounded-2xl border border-white/10 bg-[var(--surface)]/60 p-4">
          ...
          <div className="space-y-4">
            {workedExamples.map((ex) => (
              ...
              >
                ...
                <p className="mt-2 text-sm text-[var(--text-body)]"><span className="font-black text-[var(--foreground)]">Method: </span>{ex.method}</p>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            {[
              ...
              <aside key={String(title)} className="rounded-2xl border border-amber-400/25 bg-amber-400/[0.06] p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-[var(--text-body)]">{String(body)}</p>
              </aside>
            ))}
          </div>
        </Section>
            ...
            {practiceProblems.map((p, i) => (
              <article key={i} className="rounded-2xl border border-white/10 bg-[var(--surface)]/60 p-4">
                ...
                <div className="mt-3 rounded-xl border border-emerald-300/20 bg-emerald-300/[0.06] p-3">
                  ...
                  <p className="mt-1 text-sm leading-6 text-[var(--foreground)]">{p.a}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        <section className="rounded-[2rem] border border-white/10 bg-[var(--background)] p-6">
          ...
          <ul className="mt-3 space-y-1 text-sm text-[var(--text-body)]">
