...
export default function IsomerismPart79() {
  return <main className="min-h-screen bg-[#08111f] px-4 py-8 text-white sm:px-6 sm:py-12"><article className="mx-auto max-w-6xl"><Link href="/learn/isomerism" className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-2 text-sm font-bold text-cyan-100">Isomerism</Link><header className="mt-6 rounded-3xl border border-cyan-300/15 bg-[#08111f] p-6 sm:p-9"><p className="text-xs font-black uppercase tracking-[.24em] text-cyan-200">Isomerism source · page 69 · Part 79</p><h1 className="mt-3 text-3xl font-black sm:text-5xl">Higher cumulenes and spiranes: source decision bank</h1><p className="mt-4 max-w-3xl text-[var(--text-body)]">This page extends the cumulene parity rule to the individual source chains, then applies the same terminal-plane test to the source’s spirane set. Every marked decision needs both a suitable plane relationship and non-equivalent terminal groups.</p></header><section className="mt-7"><h2 className="text-2xl font-black text-cyan-100">A. Higher cumulenes</h2><div className="mt-4 grid gap-4 md:grid-cols-2">{cumulatedCases.map(item => <article key={item.chain} className="rounded-2xl border border-white/10 bg-white/[.035] p-5"><h3 className="font-bold text-white">{item.chain}</h3><CumulatedStructure chain={item.chain} active={item.result === "Shows GI"} /><p className={item.result === "Shows GI" ? "text-sm font-bold text-emerald-300" : "text-sm font-bold text-rose-300"}>{item.result}</p><p className="mt-2 text-sm leading-6 text-[var(--text-body)]">{item.reason}</p></article>)}</div></section><section className="mt-9"><h2 className="text-2xl font-black text-cyan-100">B. Geometrical isomerism in spiranes</h2><p className="mt-2 text-sm leading-6 text-[var(--text-body)]">At the spiro junction, successive ring planes alternate. Trace from one terminal ring to the other; then test whether the terminal substituents are distinguishable. The following cards preserve the individual accept/reject patterns on the source page.</p><div className="mt-4 grid gap-4 md:grid-cols-2">{spiraneCases.map((item, index) => <article key={`${item.label}-${index}`} className="rounded-2xl border border-white/10 bg-white/[.035] p-5"><h3 className="font-bold text-white">{item.label}</h3><SpiraneStructure item={item} /><p className="text-sm leading-6 text-[var(--text-body)]"><b className={item.result === "Shows GI" ? "text-emerald-300" : "text-rose-300"}>{item.result}.</b> {item.reason}</p></article>)}</div></section><aside className="mt-7 rounded-2xl border border-amber-300/25 bg-amber-300/[.07] p-5 text-sm leading-6 text-amber-50"><b>Examination check:</b> do not apply only an odd/even shortcut. First confirm that the terminal groups or paths can be compared as two different alternatives. An even/odd plane relationship cannot create E/Z-style isomerism by itself.</aside></article></main>;
}
